...
class BaseDataset(data.Dataset):
    ...
    def get_mean_std(self):
        ...
        if not os.path.isfile(mean_std_cache):
            ...
            number_augmentations = self.opt.number_augmentations
            self.opt.number_augmentations = 1
            mean, std = np.array(0), np.array(0)
            for i, data in enumerate(self):
                if i % 500 == 0:
                    print('{} of {}'.format(i, self.size))
                half_edge_features = data['half_edge_features']
                mean = mean + half_edge_features.mean(axis=1)
                std = std + half_edge_features.std(axis=1)
            mean = mean / (i + 1)
            std = std / (i + 1)
            ...
            self.opt.number_augmentations = number_augmentations
