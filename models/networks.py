...
def define_loss(opt):
    if opt.dataset_mode == 'classification':
        ...
        # -1 is used in padding: if a halfedge does not exist, -1 is entered in the labels. This entries should be ignored
...
class MResConv(nn.Module):
    def __init__(self, in_channels, out_channels, nbh_size, skips=1):
        super(MResConv, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.skips = skips
        self.conv0 = half_edge_mesh_conv.HalfEdgeMeshConv(self.in_channels, self.out_channels, kernel_width=nbh_size + 1,
                                                          bias=False)
        for i in range(self.skips):
            setattr(self, 'bn{}'.format(i + 1), nn.BatchNorm2d(self.out_channels))
            setattr(self, 'conv{}'.format(i + 1),
                    half_edge_mesh_conv.HalfEdgeMeshConv(self.out_channels, self.out_channels, kernel_width=nbh_size + 1,
                                                         bias=False))
...
class MeshEncoderDecoder(nn.Module):
    ...
    def __init__(self, pools, down_convs, up_convs, nbh_size, blocks=0, transfer_data=True):
        ...
        self.encoder = MeshEncoder(pools, down_convs, nbh_size, blocks=blocks)
        unrolls = pools[:-1].copy()
        unrolls.reverse()
        self.decoder = MeshDecoder(unrolls, up_convs, nbh_size, blocks=blocks, transfer_data=transfer_data)
...
class DownConv(nn.Module):
    def __init__(self, in_channels, out_channels, nbh_size, blocks=0, pool=0):
        super(DownConv, self).__init__()
        self.bn = []
        self.pool = None
        self.conv1 = half_edge_mesh_conv.HalfEdgeMeshConv(in_channels, out_channels, kernel_width=nbh_size + 1)
        self.conv2 = []
        for _ in range(blocks):
            self.conv2.append(half_edge_mesh_conv.HalfEdgeMeshConv(out_channels, out_channels, kernel_width=nbh_size + 1))
            self.conv2 = nn.ModuleList(self.conv2)
        for _ in range(blocks + 1):
            self.bn.append(nn.InstanceNorm2d(out_channels))
            self.bn = nn.ModuleList(self.bn)
        if pool:
            self.pool = half_edge_mesh_pool.HalfEdgeMeshPool(pool)
...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, nbh_size, blocks=0, unroll=0, residual=True,
        ...
        self.unroll = None
        self.transfer_data = transfer_data
        self.up_conv = half_edge_mesh_conv.HalfEdgeMeshConv(in_channels, out_channels, kernel_width=nbh_size + 1)
        if transfer_data:
            self.conv1 = half_edge_mesh_conv.HalfEdgeMeshConv(2 * out_channels, out_channels, kernel_width=nbh_size + 1)
        else:
            self.conv1 = half_edge_mesh_conv.HalfEdgeMeshConv(out_channels, out_channels, kernel_width=nbh_size + 1)
        self.conv2 = []
        for _ in range(blocks):
            self.conv2.append(half_edge_mesh_conv.HalfEdgeMeshConv(out_channels, out_channels, kernel_width=nbh_size + 1))
            self.conv2 = nn.ModuleList(self.conv2)
...
class MeshEncoder(nn.Module):
    def __init__(self, pools, convs, nbh_size, fcs=None, blocks=0, global_pool=None):
        ...
        for i in range(len(convs) - 1):
            if i + 1 < len(pools):
                ...
                pool = 0
            self.convs.append(DownConv(convs[i], convs[i + 1], nbh_size, blocks=blocks, pool=pool))
...
class MeshDecoder(nn.Module):
    def __init__(self, unrolls, convs, nbh_size, blocks=0, batch_norm=True, transfer_data=True):
        ...
        for i in range(len(convs) - 2):
            if i < len(unrolls):
                ...
                unroll = 0
            self.up_convs.append(UpConv(convs[i], convs[i + 1], nbh_size, blocks=blocks, unroll=unroll,
                                        batch_norm=batch_norm, transfer_data=transfer_data))
        self.final_conv = UpConv(convs[-2], convs[-1], nbh_size, blocks=blocks, unroll=False,
