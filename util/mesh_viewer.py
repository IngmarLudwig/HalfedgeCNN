...
face_colors = (r2h((15, 167, 175)), r2h((230, 81, 81)), r2h((142, 105, 252)), r2h((248, 235, 57)),
               r2h((51, 159, 255)), r2h((225, 117, 231)), r2h((97, 243, 185)), r2h((161, 183, 196)))
...
def surfaces(mesh, plot):
    vs, faces, edges = mesh
    # In the faces array in place 3 there might be the segmentation. therefore split.
    vtx = vs[faces[:, 0:3]]

    edgecolor = edge_color if not len(edges) else 'none'

    # Case there is a face based segmentation
    tri = None
    if faces.shape[1] >= 4:
        colors = np.array([face_colors[int(seg) % len(face_colors)] +'55' for seg in faces[:, 3]])
        alpha = 1.0
        tri = a3.art3d.Poly3DCollection(vtx, facecolors=colors, edgecolors=edgecolor, linewidths=.5, linestyles='dashdot', alpha=alpha)
    # Otherwise just use a single color for all faces and make them transparent
    else:
        colors = surface_color  +'55'
        tri = a3.art3d.Poly3DCollection(vtx, facecolors=colors, edgecolors=edgecolor, linewidths=.5, linestyles='dashdot')
...
def parse_obje(obj_file, scale_by):
    ...
    with open(obj_file) as f:
        for line in f:
            ...
            elif splitted_line[0] == 'e' or splitted_line[0] == 'he':
                if len(splitted_line) >= 4:
                    edge_v = [int(c) for c in splitted_line[1:-1]]
